...
async function resetFork(opts) {
    ...
    try {
        await executeCommand(checkoutCommand).then(() =>
            log(`Checking out to ${branch}`)
        );
        await executeCommand(fetchCommand).then(() =>
            log(`Fetching ${upstream}`)
        );
        await executeCommand(resetCommand).then(() => log(`Resstting`));
        await executeCommand(pushCommand).then(() => log(`Pushing to origin`));
    } catch (err) {
        errorHandler(err);
    }
}
